...
```ts
import { defaultProtoCodec, convertToProtoFactory } from '@leapwallet/buffer-boba';
import { sdkName } from 'some-library';

defaultProtoCodec.registerProtoFactory(sdkName.someMessage.typeUrl, convertToProtoFactory(sdkName.someMessage));
```
